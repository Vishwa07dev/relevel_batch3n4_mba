...
const isAdminOrOwner = async (req, res, next) => {
  try {
    ...
    if (
      ...
      callingUser.userId == req.params.userId
    ) {
      next();
    } else {
...
const isTheatreOwnerOrAdmin = async (req, res, next) => {
  try {
    const allowedUserTypes = [
      constants.userTypes.theatre_owner,
    ...
    console.log(
      "#### Error while authenticating the user info ####",
      err.message
    );
    return res.status(500).send({
...
const isValidTheatreOwner = async (req, res, next) => {
  try {
    if (
      req.user.userType == constants.userTypes.theatre_owner ||
      req.user.userType == constants.userTypes.admin
    ) {
      const theatre = await Theatre.findOne({ _id: req.params.id });
      if (
        theatre.ownerId.equals(req.user._id) ||
        req.user.userType == constants.userTypes.admin
      ) {
        next();
      } else {
        return res.send(403).send({
          message:
            "Only the owner of this theatre is allowed to make this call",
        });
      }
    }
  } catch (err) {
