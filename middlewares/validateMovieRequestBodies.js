...
const newMovieBody = (req, res, next) => {
  try {
    ...
    console.log(
      "#### Error while velidating new movie request body ##### ",
      err.message
    );
    res.status(500).send({
...
const editMovieBody = (req, res, next) => {
  try {
    ...
    if (
      ...
      !AllowedReleaseStatuses.includes(req.body.releaseStatus)
    ) {
      return res.status(400).send({
        message: "Failed ! Invalid movie release status provided",
      });
    ...
    console.log(
      "#### Error while velidating edit movie request body ##### ",
      err.message
    );
    res.status(500).send({
