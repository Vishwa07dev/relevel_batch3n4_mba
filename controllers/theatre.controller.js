const Theatre = require('../models/theatre.model')
 ...
 exports.addTheatre = async (req,res)=>{
     try{
         ...
         console.log(`#### New Movie '${theatre.name}' created ####`);
         res.status(201).send(theatre);

 
        }catch(err){
         console.log("#### Error while ading theatre #### ", err);
         res.status(500).send({
...
exports.updateTheater = async (req,res)=>{
    try{
        ...
        const updateddata = await movie.save();
...
exports.getSinglTheatre = async (req,res)=>{
